-- 合并自 migrations/（37 个文件）。对新库（空 schema）执行本文件即可。
...
-- 业务正常用 authenticated（RLS 拦截）；此处仅给 service_role 补表级权限，-- 供数据迁移直连读取（service_role 凭 BYPASSRLS 绕过 RLS 策略，但表级 GRANT 不足仍会被拒）。
