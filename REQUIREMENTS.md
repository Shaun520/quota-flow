...
```
+-----------------------------------------------------------+
| Quota-Flow  [调度台] [历史] [团队] [设置]    [等效 110/120] |
+-----------------------------------------------------------+
|                                                           |
|              当前 Tab 的完整功能区域                         |
|                                                           |
...
**顶部**：
- 左：Logo Quota-Flow
- 中：主 Tab 栏 [调度台] [历史] [团队] [设置]
- 右：额度徽章 "等效 110 / 120"（常驻显示团队总池剩余等效次数，点击跳调度台）
...
**区块 1：额度总览**
- 自适应网格，每家厂商一张卡片
- 显示：厂商 logo + 今日剩余/总额度（原生单位，如"720/800 灵感值"）+ 等效次数括号 + 状态徽章（active/exhausted/degraded/offline）
- 超过 6 家折叠，显示"展开全部 N 家"按钮
- 点卡片展开：这家厂商所有账号明细（每个账号的原生单位剩余、健康状态、启用/禁用开关、成功率）
...
**区块 2：快速生成**
- 模式选择：[文字生视频] [图生视频] [视频生视频] [多图生视频]
- 高级参数折叠区：时长（秒）滑块、分辨率下拉、模型下拉（基于当前选中模式支持的参数）
- prompt 输入框（textarea）
- 附件上传区（拖拽/点击/粘贴，单文件 50MB，单次 5 个）
- **预计消耗预览**：根据当前参数 + 模式实时显示"豆包 1 次 / 即梦 80 灵感值 / 等效 1 次"
- 厂商偏好下拉：自动 / 指定某家 / 排除某家
- [生成] 大按钮

**区块 3：最近任务**
- 每行一个任务：时间、厂商、模式、状态、质量分、消耗（原生单位 + 等效次数）、视频缩略图
...
完整任务记录，可筛选可批量：
- 筛选器：厂商、状态、日期范围、搜索
- 表格列：勾选框、时间、厂商、模式、状态、质量、消耗（原生+等效）、操作
- 批量勾选：批量下载、批量删除
- 导出 CSV
- 分页

### 11.4 Tab 3：团队（团队订阅用户可见）

- 团队信息：名称、套餐、到期时间、今日用量（等效次数）
- 成员管理：列表、角色、今日消费、邀请/移除
- 团队公共 ApiKeys：绑定的 cookie/key 清单（admin 可见明文）

### 11.5 Tab 4：设置

- 我的 ApiKeys：绑各家 cookie/apikey + 测试 + 删除 + 启用/禁用开关
- 账号健康（cookie 状态）：每家每个账号的 cookie 健康状态、一键重新登录、一键全部健康检查
- 偏好设置：默认路由策略、质量阈值、成员自带 key 默认进总池/仅本人
- 订阅：当前套餐、升级、取消
- 关于：版本、GitHub Issues、邮箱支持、赞助

### 11.6 续生成机制（不靠对话页）

任务列表每行有菜单：
- 下载：下载到本地
- 续生成：跳到调度台，prompt 框预填上一轮内容，附件自动带上轮视频
- 换家重试：强制排除上次厂商，立即重新生成
- 删除
...
- 成员管理 + 邀请
- 防滥用机制（成员日等效次数上限）
- admin 后台：团队管理 + 用户管理 + Provider 管理 + **消耗表编辑器** + 系统监控
