...
> 关联设计：[admin-system-plan.md](./后台系统规划.md)、REQUIREMENTS.md §13.4.1

---

## 1. 背景

`apps/admin` 后台骨架阶段，8 个页面均通过 `PrototypePage` 注入 `docs/设计原型` 导出的静态 HTML 字符串渲染，用户管理页同样如此——数据、筛选器、按钮全是写死的，无任何交互。
...
- [data-consistency-fix-summary.md](../数据库与额度/数据一致性修复摘要.md) — RPC 与 Supabase 迁移的先例写法
